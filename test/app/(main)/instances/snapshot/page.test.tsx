...
import Page from '@/app/(main)/instances/snapshot/page';
...
it('should render correctly', async () => {
  render(<Page />);
  expect(
    screen.getByText(/This is the instance ID you want to snapshot/i)
  ).toBeInTheDocument();
});

it('should display a message when the form is submitted successfully', async () => {
  vi.mocked(callApi).mockResolvedValue({ snapshotId: '123' });
  ...
  await waitFor(() => {
    expect(
      screen.getByText(/Saving a snapshot under the snapshot ID 123/i)
    ).toBeInTheDocument();
  });
});
