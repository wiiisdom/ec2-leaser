...
describe('ec2 snapshot', () => {
  it('snapshotInstance must fail if bad instanceId is provided', () => {
    ...
    expect(() => snapshotInstance('instanceId')).rejects.toThrowError('find the instance');
  });
  ...
  it('snapshotInstance must fail if no EBS volume Id', () => {
    ...
    expect(() => snapshotInstance('instanceId')).rejects.toThrowError('Cannot find the volumeId');
  });
