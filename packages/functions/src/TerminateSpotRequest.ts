  ...
  const getInstance = async (request: EC2.SpotInstanceRequest) => {
    ...
    try {
      const instance = await ec2
        .describeInstances({ InstanceIds: [request.InstanceId] })
        .promise();
      return instance?.Reservations?.[0];
    } catch (err) {
      throw new Error(
        `\t[${request.SpotInstanceRequestId}]: instance not existing anymore`
      );
    }
  };

  try {
    const { SpotInstanceRequests } = await ec2
      .describeSpotInstanceRequests()
      .promise();
    if (SpotInstanceRequests === undefined)
      throw new Error("Error getting Spot Instance Requests");
      ...
      SpotInstanceRequests.map(async request => {
        ...
        try {
          ...
          if (!spotInstanceRequestId) {
            info(
              `\t[${spotInstanceRequestId}]: No spotInstanceRequestId so exiting`
            );
            return 0;
          }
          ...
          if (instanceTags?.find(tag => tag.Key === "Name")) {
            info(
              `\t[${spotInstanceRequestId}]: Attached instance have a Name tag so exiting`
            );
            return 0;
          }
          ...
          const instanceId =
            instanceCreatedByRequest?.Instances?.[0]?.InstanceId;
          info(
          ...
          if (instanceId) {
            try {
              ...
              throw new Error(
                `\tCannot terminate instance [${spotInstanceRequestId}]: ${err}`
              );
            }
          }
          return 1;
