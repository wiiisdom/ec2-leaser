                ...
                <select id="image" name="image">
                  {this.state.images.map((item, key) => (
                    <optgroup key={key} label={item.name}>
                      {item.launchTemplates.map((launchTemplate, key2) => (
                        <option key={key2} value={launchTemplate.id}>
                          {launchTemplate.name} ({launchTemplate.description})
                        </option>
                      ))}
                    </optgroup>
                  ))}
